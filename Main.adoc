= {subject}
:subject: DO180 Supplmentary Activities
:description:  Extra activities to use with the DO180 course
Andrew Blum <ablum@redhat.com>
:doctype: book
:customer:  PFE
:listing-caption: Listing
:toc:
:toclevels: 7
:sectnums:
:sectnumlevels: 6
:numbered:
:chapter-label:
:pdf-page-size: LETTER
:icons: font
ifdef::backend-pdf[]
:title-page-background-image: image:images/Training_Cover.png[pdfwidth=8.0in,position=top left]
:pygments-style: tango
:source-highlighter: pygments
endif::[]
ifndef::env-github[:icons: font]
ifdef::env-github[]
:status:
:outfilesuffix: .adoc
:caution-caption: :fire:
:important-caption: :exclamation:
:note-caption: :paperclip:
:tip-caption: :bulb:
:warning-caption: :warning:
endif::[]
:revnumber: 1.0

DO180 supplementary activities are designed to expand on topics presented in the standard commercial course.  These activities can be completed within the delivery of a DO180 course in addition to the typical hands-on activities.

The content topics are based on learner feedback and questions asked during various DO180 VT deliveries.

//:sectnums!:
//include::Introduction.adoc[]

:sectnums:
include::Chapter1.adoc[Chapter 1: Introducing Container Technology]


// Appendix Section below here

//[appendix]
//include::AppendixA.adoc[]
